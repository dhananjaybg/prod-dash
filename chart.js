...
g
  ...
  .attr("x", 0 - height / 2)
  .style("text-anchor", "middle")
  .text("Production Total Quantity");
...
function buildRow(table, topping, index) {
	//console.log("build row"+ table + " ..topping"+  topping + " ..index "+ index );
...
//console.log(" in the refreshTable");
...
function buildGraph(salesTimeline) {
  ...
  var graphPromise = new Promise((resolve, reject) => {
	...
	console.log( "vomit..");
	console.log( JSON.stringify(SalesLine));
...
function refreshGraph(salesLine, path, newSalesTimeline) {
	console.log( "refreshGraph -- vomit..");
	console.log(JSON.stringify(salesLine));
	console.log(JSON.stringify(path));
	console.log(JSON.stringify(newSalesTimeline));
